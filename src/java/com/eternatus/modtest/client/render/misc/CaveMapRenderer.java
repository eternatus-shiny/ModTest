...
public class CaveMapRenderer {
    ...
    public static final RenderType CAVE_MAP_PLAYER_TEXTURE = RenderType.text(new ResourceLocation(References.MODID, "textures/misc/map/cave_map_player.png"));
    public static final RenderType CAVE_MAP_PLAYER_DIRECTION_TEXTURE = RenderType.text(new ResourceLocation(References.MODID, "textures/misc/map/cave_map_player_direction.png"));
        ...
        if (registry != null && mapBiomes.length >= 128 * 128) {
            for (int i = 0; i < 128; ++i) {
                for (int j = 0; j < 128; ++j) {
                    ...
                    int biomeColor = biome == null ? DefaultMapBackgrounds.DEFAULT.getMapColor(i, j) : biome.is(Registration.CAVE_MAP_BORDER_ON) && isBorderBiome(registry, biome, i, j) ? DefaultMapBackgrounds.BORDER.getMapColor(i, j) : getBiomeColor(biome, j, i);
                    int r = FastColor.ARGB32.red(biomeColor);
                    int g = FastColor.ARGB32.green(biomeColor);
                    int b = FastColor.ARGB32.blue(biomeColor);
                    double edge = Math.sqrt((i - 64) * (i - 64) + (j - 64) * (j - 64)) / 128;
                    int alpha = transparent ? Math.max(255 - (int) (255F * (edge * edge)), 10) : 255;
                    this.texture.getPixels().setPixelRGBA(j, i, FastColor.ARGB32.color(alpha, b, g, r));
                }
            }
        }
        this.texture.upload();
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, ItemStack map, boolean fullFrame, int light) {
        ...
        if (!fullFrame) {
            poseStack.pushPose();
            double dimensionScale = Minecraft.getInstance().player.getLevel().dimensionType().coordinateScale();
